<template>
    <div class="min-h-screen flex items-center justify-center bg-gray-100"> <!-- Fond blanc cassé -->
        <div class="bg-gray-50 p-8 rounded-lg shadow-lg w-full max-w-md"> <!-- Conteneur fond gris clair -->
            <h2 class="text-2xl font-bold mb-6 text-center text-gray-800">Login</h2> <!-- Titre avec texte gris foncé -->
            <form @submit.prevent="login">
                ...
                <div class="mb-6">
                    <label class="block text-gray-600 text-sm font-bold mb-2" for="password">Password</label> <!-- Label en gris moyen -->
...
export default {
    ...
    methods: {
        async login() {
            try {
                const response = await this.$http.post('http://localhost/api/users/login', {
                    email: this.email,
